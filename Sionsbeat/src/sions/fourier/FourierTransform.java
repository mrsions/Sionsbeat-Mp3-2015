/*
 * Copyright (c) 2007 - 2008 by Damien Di Fede <ddf@compartmental.net> This
 ...
 * equal to the Nyquist frequency, which in this case is 22100, so the bandwidth
 ...
 * {
 * 	// draw the line for frequency band i, scaling it by 4 so we can see it
 * 	// a bit better
 ...
 * them to the frequency domain. If you call the <code>window()</code> function
...
public abstract class FourierTransform
{
	...
	/**
	 * A constant indicating a Hamming window should be used on sample
	 * buffers.
	 */
	...
	/**
	 * Construct a FourierTransform that will analyze sample buffers that
	 * are <code>ts</code> samples long and contain samples with a
	...
	// classes
	// because the size of the arrays will depend on the implementation
	// being used
	...
	/**
	 * Sets the number of averages used when computing the spectrum and
	 * spaces the averages in a linear manner. In other words, each average
	 * band will be <code>specSize() / numAvg</code> bands wide.
	 * 
	 * @param numAvg
	...
	/**
	 * Sets the number of averages used when computing the spectrum based on
	 * the minimum bandwidth for an octave and the number of bands per
	 * octave. For example, with audio that has a sample rate of 44100 Hz,
	 * <code>logAverages(11, 1)</code> will result in 12 averages, each
	 * corresponding to an octave, the first spanning 0 to 11 Hz. To ensure
	 * that each octave band is a full octave, the number of octaves is
	 * computed by dividing the Nyquist frequency by two, and then the
	 * result of that by two, and so on. This means that the actual
	 * bandwidth of the lowest octave may not be exactly the value
	 * specified.
	 * 
	 * @param minBandwidth
	 ...
	 * transform. If an invalid window is asked for, an error will be
	 * reported and the current window will not be changed.
	 * 
	 * @param which
	...
	/**
	 * Returns the length of the time domain signal expected by this
	 * transform.
	 * 
	 * @return the length of the time domain signal expected by this
	 *         transform
	 */
	public int timeSize ()
	{
	 ...
	 * words, the number of frequency bands produced by this transform. This
	 * is typically equal to <code>timeSize()/2 + 1</code>, see above for an
	 * explanation.
	 * 
	 ...
	 * should be noted that the bandwidth of the first and last frequency
	 * bands is half as large as the value returned by this function.
	 * 
	...
	/**
	 * Sets the amplitude of the <code>i<sup>th</sup></code> frequency band
	 * to <code>a</code>. You can use this to shape the spectrum before
	 * using <code>inverse()</code>.
	 * 
	 * @param i
	...
	/**
	 * Scales the amplitude of the <code>i<sup>th</sup></code> frequency
	 * band by <code>s</code>. You can use this to shape the spectrum before
	 * using <code>inverse()</code>.
	 * 
	 * @param i
	 *                the frequency band to modify
	...
	public float indexToFreq ( int i )
	{
		...
		// special case: the width of the first bin is half that of the
		// others.
		// so the center frequency is a quarter of the way.
		if ( i == 0 ) return bw * 0.25f;
		// special case: the width of the last bin is half that of the
		// others.
	...
	public float getAverageCenterFrequency ( int i )
	{
		if ( whichAverage == LINAVG )
		{
			// an average represents a certain number of bands in
			// the spectrum
			int avgWidth = (int) spectrum.length / averages.length;
		...
		else if ( whichAverage == LOGAVG )
		{
			...
			freqStep = ( hiFreq - lowFreq ) / avgPerOctave;
			// figure out the low frequency of the band we care
			// about
			float f = lowFreq + offset * freqStep;
			// the center of the band will be the low plus half the
			// width
			return f + freqStep / 2;
		}
	 ...
	 * @param startAt
	 *                the index to start at in the buffer. there must be at
	 *                least timeSize() samples between the starting index
	 *                and the end of the buffer. If there aren't, an error
	 *                will be issued and the operation will not be
	 *                performed.
	 * 
	 */
	public void forward ( float[] buffer, int startAt )
	{
		if ( buffer.length - startAt < timeSize ) { throw new IllegalArgumentException( "FourierTransform.forward: not enough samples in the buffer between " + startAt + " and " + buffer.length + " to perform a transform." ); }
	...
	/**
	 * Performs an inverse transform of the frequency spectrum and places
	 * the result in <code>buffer</code>.
	 * 
	 * @param buffer
	 *                the buffer to place the result of the inverse
	 *                transform in
	 */
	public abstract void inverse ( float[] buffer );

	/**
	 * Performs an inverse transform of the frequency spectrum represented
	 * by freqReal and freqImag and places the result in buffer.
	 * 
	 * @param freqReal
	...
	/**
	 * @return the imaginary part of the last FourierTransform.forward()
	 *         call.
	 */
