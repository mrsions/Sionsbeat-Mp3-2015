...
public class FFT extends FourierTransform
{
	 ...
	 * <code>timeSize</code> long and have been recorded with a sample rate
	 * of <code>sampleRate</code>. <code>timeSize</code> <em>must</em> be a
	...
	// bit reversing is not necessary as the data will already be bit
	// reversed
	private void fft ()
	{
	...
	{
		if ( buffer.length != timeSize ) { throw new IllegalArgumentException( "FFT.forward: The length of the passed sample buffer must be equal to timeSize()." ); }
		doWindow( buffer );
	 ...
	 * @param buffImag
	 *                the imaginary part of the time domain signal to
	 *                transform
	 */
	public void forward ( float[] buffReal, float[] buffImag )
	{
		if ( buffReal.length != timeSize || buffImag.length != timeSize ) { throw new IllegalArgumentException( "FFT.forward: The length of the passed buffers must be equal to timeSize()." ); }
	...
	{
		if ( buffer.length > real.length ) { throw new IllegalArgumentException( "FFT.inverse: the passed array's length must equal FFT.timeSize()." ); }
		// conjugate
